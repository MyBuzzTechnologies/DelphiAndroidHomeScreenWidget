...
@rmdir /Q /S %HostAppPath%\Android\src
@rmdir /Q /S %HostAppPath%\Android\obj
@mkdir %HostAppPath%\Android\src
@mkdir %HostAppPath%\Android\obj
@echo -----------

REM -- Create R.java for resources for Widget
%SDKAaptPath% package -f -m -M %1\Widget\AndroidManifest.xml -I %SDKApiLevelPath% -S %1\Widget\res -J %HostAppPath%\Android\src

REM -- Copy any additional java files across ready for compilation
copy %1\Widget\src\*.java %HostAppPath%\Android\src\com\embarcadero\widgettestapp\
@echo -----------

REM -- Compile R.java into R$ classes for Widget
%JDKPath%\javac -verbose -d %HostAppPath%\Android\obj -classpath %SDKApiLevelPath%;%HostAppPath%\Android\obj -sourcepath %HostAppPath%\Android\src %HostAppPath%\Android\src\com\embarcadero\widgettestapp\*.java
@echo -----------
